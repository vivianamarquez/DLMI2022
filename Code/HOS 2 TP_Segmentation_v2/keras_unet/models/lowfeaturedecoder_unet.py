...
def lowfeaturedecoder_unet(
    ...
    for l in range(num_layers):
        x = conv2d_block(inputs=x, filters=filters, kernel_size=kernel_size, activation=cnn_activation, use_batch_norm=use_batch_norm, dropout=dropout, kernel_regularizer=kernel_regularizer)
        down_layers.append(x)
        x = MaxPooling2D((2, 2)) (x)
        dropout += dropout_change_per_layer
        filters = filters*2 # double the number of filters with each layer

    x = conv2d_block(inputs=x, filters=filters, activation=cnn_activation, use_batch_norm=use_batch_norm, dropout=dropout, kernel_regularizer=kernel_regularizer)
    ...
    x = conv2d_block(inputs=x, filters=num_classes*filters//original_filters, activation=cnn_activation, use_batch_norm=use_batch_norm, dropout=dropout, kernel_regularizer=kernel_regularizer)
